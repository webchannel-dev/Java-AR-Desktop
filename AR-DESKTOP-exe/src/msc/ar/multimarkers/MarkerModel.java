...
{
  private final String MARKER_DIR = "src\\Data\\";
  ...
  { 
    ...
    try {
      ...
      markerInfo.loadARPattFromFile(markerName);  // load marker image
    }
  ...
  // detected marker so update model's moveTG
  {
    ...
    //Added rotation to models so it apprears to be rotating --sample
     double deg = rotfactor * (Math.PI/6);
     rotfactor = (rotfactor+1)%7;    
